#[test_only]
module Auctions_on_Aptos::auction_tests{
    use std::signer;
    use std::string;
    ...
    use Auctions_on_Aptos::Auction;

    struct AuctionTest has key{
        mint_cap: MintCapability<AptosCoin>,
        burn_cap: BurnCapability<AptosCoin>
    }


    #[test(admin = @Auctions_on_Aptos)]
    public entry fun test_init(admin: &signer){
        account::create_account_for_test(signer::address_of(admin));
        Auction::initialize_auction(admin);
    }
    ...
    #[test(seller = @0x123, creator = @0x456, admin = @Auctions_on_Aptos, aptos_framework = @aptos_framework,
        bidder = @0x222
    )]
    public entry fun test_claim_coins(bidder: &signer, seller: &signer, admin: &signer, creator:&signer, aptos_framework: &signer){
        ...
        Auction::claim_coins(seller, signer::address_of(creator), b"COLLECTION", b"TokenName");
    ...
    public entry fun test_bid_nft(bidder: &signer, seller: &signer, admin: &signer, creator:&signer, aptos_framework: &signer) {
        ...
        Auction::bid(bidder, signer::address_of(creator), b"COLLECTION", b"TokenName", bid);

        move_to(admin, AuctionTest{
            mint_cap,
            burn_cap
        });
    }
    ...
    public entry fun test_sell_nft(seller: &signer, admin: &signer, creator:&signer, aptos_framework: &signer){
        ...
        Auction::sell_nft(seller, signer::address_of(creator), collection_name, name, sell_price, duration);
        ...
        //
        assert!(Auction::isLockedToken(created_token_id), 1);
    }
